...
export function Enhanced3DViewer({ 
  ...
  useEffect(() => {
    ...
    const scale = 15 * viewState.zoom;
    const offsetX = width * 0.5 + viewState.panX;
    const offsetY = height * 0.7 + viewState.panY;
    ...
    quads.forEach(quad => {
      ...
      if (showLighting) {
        const lightFactor = 0.5 + (quad.depth / 100) * 0.5;
        const rgb = quad.color.match(/\d+/g)?.map(Number) || [255, 255, 255];
        ctx.fillStyle = `rgba(${rgb[0] * lightFactor}, ${rgb[1] * lightFactor}, ${rgb[2] * lightFactor}, 0.9)`;
      } else {
